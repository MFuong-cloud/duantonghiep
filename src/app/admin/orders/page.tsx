  ...
  return (
    <div className="bg-white dark:bg-[#1f1f1f] text-gray-800 dark:text-gray-100 p-6 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 transition-colors duration-300">
      <div className="flex justify-between items-center mb-5 flex-wrap gap-2">
        <h2 className="text-2xl font-bold text-blue-600">Quản lý đơn đặt hàng</h2>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        <div className="relative flex-1 min-w-[220px]">
          ...
          <input
            ...
            className="w-full bg-gray-100 dark:bg-[#2a2a2a] border border-gray-300 dark:border-gray-700 pl-9 p-2 rounded-md focus:ring-2 focus:ring-blue-500 outline-none placeholder-gray-400 dark:placeholder-gray-500"
          />
        </div>
          ...
          <thead className="bg-gray-100 dark:bg-[#2a2a2a]">
            <tr>
              {["Mã đơn", "Họ và tên", "SĐT", "Số người", "Ngày đặt", "Giờ đặt", "Tổng tiền", "Trạng thái", "Hành động"].map((h) => (
                <th key={h} className="p-3 text-left text-blue-500 font-semibold">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
                    ...
                    >
                      ...
                      <Dialog>
                        ...
                        <DialogContent className="bg-white dark:bg-[#1f1f1f] text-gray-800 dark:text-gray-100 rounded-lg">
                          <DialogHeader>
                            <DialogTitle className="text-blue-500 text-xl">Chi tiết đơn hàng {order.id}</DialogTitle>
                          </DialogHeader>
      ...
      <div className="flex justify-center mt-4 gap-2">
        {Array.from({ length: totalPages }, (_, i) => (
          <button key={i} onClick={() => setCurrentPage(i + 1)}
            className={`px-3 py-1 rounded-md ${currentPage === i + 1 ? "bg-blue-600 text-white" : "bg-gray-200 dark:bg-[#2a2a2a]"}`}>
            {i + 1}
          </button>
        ))}
      </div>
