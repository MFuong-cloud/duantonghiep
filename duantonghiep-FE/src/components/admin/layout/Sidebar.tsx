...
export default function Sidebar({ collapsed = false }: Props) {
  ...
  const activeClass =
    "bg-blue-50 dark:bg-[#1E293B] text-blue-600 dark:text-blue-300 shadow-sm";
  const hoverClass =
  ...
  return (
    <div className="h-full flex flex-col justify-start p-4 bg-white dark:bg-[#0B0B0B] border-r border-gray-200 dark:border-[#1F2937] shadow-lg">
      ...
      <Link
        href="/admin"
        className={`flex items-center gap-3 mb-6 transition hover:opacity-80 ${
        ...
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <motion.span
                layoutId={isActive ? "activeLink" : undefined}
                className="absolute inset-0 bg-blue-100/20 dark:bg-blue-900/10 rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-200"
              />
            </Link>
          );
        })}
